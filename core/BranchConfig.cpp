...
BranchConfig BranchConfig::CloneAndMerge(const BranchConfig& attached) const {
  ...
  auto result = Clone((name1 + "_" + name2).c_str(), type1);

  for (const auto& field : attached.AnalysisTree::VectorConfig<float>::map_) {
    result.AddField<float>((name2 + "_" + field.first).c_str(), (name2 + ": " + field.second.title_).c_str());
  }
  for (const auto& field : attached.AnalysisTree::VectorConfig<int>::map_) {
    result.AddField<int>((name2 + "_" + field.first).c_str(), (name2 + ": " + field.second.title_).c_str());
  }
  for (const auto& field : attached.AnalysisTree::VectorConfig<bool>::map_) {
    result.AddField<bool>((name2 + "_" + field.first).c_str(), (name2 + ": " + field.second.title_).c_str());
  }
