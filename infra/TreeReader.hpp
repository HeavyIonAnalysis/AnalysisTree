...
static inline std::map<std::string, void*> GetPointersToBranches(TChain* t, const Configuration& config,
  ...
  for (const auto& branch : names){  // Init all pointers to branches
    ...
    switch(branch_config.GetType()){
      case DetType::kTrack :       { branch_ptr = new TrackDetector;   break; }
      case DetType::kHit :         { branch_ptr = new HitDetector ;    break; }
      case DetType::kEventHeader : { branch_ptr = new EventHeader;     break; }
      case DetType::kParticle :    { branch_ptr = new Particles;       break; }
      case DetType::kModule :      { branch_ptr = new ModuleDetector ; break; }
    }
